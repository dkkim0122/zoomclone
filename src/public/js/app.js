...
const enterForm = welcome.querySelector("form");  // welcome이라는 id에서 "form"과 일치하는 element를 반환한다.
...
function handleRoomSubmit(event){
    ...
    const roomNameInput = enterForm.querySelector("#roomName");  // form 태그에서 input DOM 찾기
    ...
    socket.emit("enter_room", roomNameInput.value, nicknameInput.value, showRoom); // enter_room이라는 event를 백엔드로 emit한다.
    roomName = roomNameInput.value;  // 얘가 showroom보다 먼저 실행됨. showroom은 callback 함수이므로!!
    roomNameInput.value = "";
...
socket.on ("room_change", (rooms) =>{
    ...
    roomList.innerHTML = "";  // ul을 항상 비워준다. 매번 누군가가 들어가면 

    if (rooms.length === 0){  // room 나갈 때
        return;
    }
    console.log(rooms);
    rooms.forEach(room => {
        const li = document.createElement("li");
        li.innerText = room;
        roomList.append(li);  // 비어 있는 roomList(ul)에 li를 붙인다.
    })
});